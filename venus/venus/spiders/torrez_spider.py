...
from venus.dwItems.goods_item import GoodsItem
from venus.dwItems.page_item import PageItem
from venus.dwItems.site_item import SiteItem
from scrapy_redis.spiders import RedisSpider
from venus.utils.log import logger
from venus.utils.minio_util import MinioUtil
from venus.utils.redis import RedisTool
from venus.utils.request import SeleniumRequest
...
class ToRReZSpider(RedisSpider):
    ...
    drugs_url_base = "http://venus7rnkrbtdctbdtmvydn5kvjiaqmf2kgtzfup4muavc6wx4clwyqd.onion/index.php?route=product/category&path=222"
    page_url_base = "http://venus7rnkrbtdctbdtmvydn5kvjiaqmf2kgtzfup4muavc6wx4clwyqd.onion/index.php?route=product/category&path="
    start_url_base = "http://venus7rnkrbtdctbdtmvydn5kvjiaqmf2kgtzfup4muavc6wx4clwyqd.onion/index.php?route=common/home"
    domain_url = "http://venus7rnkrbtdctbdtmvydn5kvjiaqmf2kgtzfup4muavc6wx4clwyqd.onion"
    start_urls = [drugs_url_base]
    ...
    def parse(self, response):
        ...
        if response.url == self.drugs_url_base:
            ...
            page_numbers = [222, 225, 226, 227, 228, 229, 230, 231, 232, 233, 234, 235, 236, 237]
            for i in page_numbers:
                dont_filter = True if i == 222 else False
    ...
    def parse_table_data(self, response):
        ...
        main_products_div = soup.find('div', class_='main-products product-grid')
        product_layouts = main_products_div.find_all('div', class_='product-layout has-extra-button')

        for row in product_layouts:
            # items = row.find_all('td')
            #
            # class_name = items[0].attrs['class'][0] maxthumb？
            class_name = row.find('div').attrs['class'][0]
            # href = items[0].find('a').get('href')
            href = row.find('a').get('href')
            # imgUrl = items[0].find('img').attrs['src']
            imgUrl = row.find('img').attrs['src']
            # name = items[1].contents[1].contents[0]
            caption_div = row.find('div', class_='caption')
            name = caption_div.find('a').get_text()
            # categories = items[1].find('div').find_all('a')
            print("torrez_spider.py",imgUrl,name)
            #拿不到
